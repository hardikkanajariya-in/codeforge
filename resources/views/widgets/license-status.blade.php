<div class="fi-wi-stats-overview-stat relative rounded-xl bg-white p-6 shadow-sm ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10">
    ...
    <div class="flex items-center gap-4">
        <div class="flex-shrink-0">
            <div class="flex h-12 w-12 items-center justify-center rounded-full bg-{{ $color }}-100 dark:bg-{{ $color }}-500/20">
                <x-filament::icon
                    :icon="$icon"
        ...
        <div class="flex-shrink-0 flex flex-col gap-2">
            @if(in_array($licenseData['status'], ['expired', 'invalid', 'expiring']) && isset($licenseData['action_url']))
                <x-filament::button
                    :href="$licenseData['action_url']"
                    target="_blank"
                    color="{{ $licenseData['status'] === 'expiring' ? 'warning' : 'primary' }}"
                    size="sm"
                >
